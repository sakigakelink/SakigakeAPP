...
| 前月引き継ぎ | 前月末シフト状態を月初の制約に反映（**初期配置**） |
| 希望シフト厳守 | 全ての指定（assign）・除外（avoid）希望はハード制約（**初期配置**、緩和不可） |
...
| 夜勤帯の新人重複禁止 | 準夜帯・深夜帯にskillPoint=0の職員が2名以上入らない（config.json `noDoubleRookie` 設定） |

### 3.2 前月引き継ぎ詳細（初期配置）

前月確定データから以下を自動取得し、**初期配置**としてソルバーに渡す（ソルバーはこれらを動かさない）:
...
|------|-----------|------|
| 人員不足 | 500点/人 | 必要人数未達 |
| スキルポイント合計不足 | 400点/不足SP | 各時間帯のスキルポイント合計が曜日別最低値に未達（config.json `skillPointMinimumPerBand` 設定） |
