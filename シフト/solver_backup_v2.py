...
class ShiftSolver:
    ...
    def _solve_core(self, log_queue=None):
        ...
        # 夜勤制約: nightRestriction フィールドに基づく
        for s in range(self.num_staff):
            restriction = self.staff_list[s].get("nightRestriction", None)
            if restriction == "junnya_only":
                for d in range(self.num_days):
                    self.model.Add(self.shifts[(s, d, SHIFT_IDX["shinya"])] == 0)
            elif restriction == "shinya_only":
                for d in range(self.num_days):
                    self.model.Add(self.shifts[(s, d, SHIFT_IDX["junnya"])] == 0)

        # 遅出は二病棟のみ。一・三病棟は検討不要なので全スタッフ禁止して終わり
        if ward in ("1", "3"):
            for s in range(self.num_staff):
                for d in range(self.num_days):
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["late"])] == 0)
        else:
            # 二病棟: 遅出の必要人数制約
        ...
        for s in range(self.num_staff):
            ...
            if work_type == "day_only":
                for d in range(self.num_days):
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["night2"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["junnya"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["shinya"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["ake"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["late"])] == 0)
                ...
                # 夜勤専従: night2, ake, off, paid, refresh のみ許可
                for d in range(self.num_days):
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["day"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["late"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["junnya"])] == 0)
                    self.model.Add(self.shifts[(s,d,SHIFT_IDX["shinya"])] == 0)
        ...
        for s in range(self.num_staff):
            ...
            if wt == "2kohtai":
                ...
                raw_nc = self.model.NewIntVar(0, self.num_days * 2, f"raw_nc_{s}")
            ...
            nc_rate_upper = self.num_days * 2 * NC_SCALE // max_burden + 1
        ...
        # 目的関数
        if nc_list:
            mx = self.model.NewIntVar(0, self.num_days * NC_SCALE, "mx")
            mn = self.model.NewIntVar(0, self.num_days * NC_SCALE, "mn")
            self.model.AddMaxEquality(mx, nc_list)
            self.model.AddMinEquality(mn, nc_list)

            # 夜勤レンジ段階的ペナルティ（レンジが大きいほど急激に増加）
            night_range_diff = self.model.NewIntVar(0, self.num_days * NC_SCALE, "night_range_diff")
